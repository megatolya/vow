...
Promise.prototype = {
    ...
    then : function(onFulfilled, onRejected, onProgress, ctx) {
        ...
        if(!this._isRejected && !this._isFulfilled) {
            this._progressCallbacks.push({ promise : promise, fn : onProgress, ctx : ctx });
        }
    ...
    timeout : function(timeout) {
        var promise = new Promise(),
            timer = setTimeout(
                function() {
                    promise._reject(Error('timed out'));
                },
                timeout);
    ...
    resolve : function(promise) {
        this.isResolved() || this._resolve(promise);
    },
    ...
    _resolve : function(promise) {
        ...
        if(isObject(promise) || isFunction(promise)) {
            try {
                promise.then;
            }
            catch(e) {
